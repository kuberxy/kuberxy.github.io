...
我们知道 ，`log_by_lua*` 是一个请求经历的最后阶段。由于记日志跟响应内容无关，因此Nginx 通常在结束请求之后才更新访问日志。由此可见，如果我们有日志要输出，最好统一到 `log_by_lua*` 阶段。而如果我们把记日志的操作放在 `content_by_lua*` 阶段，那么将线性的增加请求的处理时间。 

> 通常，nginx的日志内容都要输送到 syslog 日志服务器，因此一般会使用[lua-resty-logger-socket](https://github.com/cloudflare/lua-resty-logger-socket)这个库。 
...
if not logger.initted() then
    local ok, err = logger.init{
        host = 'xxx',
        port = 1234,
        -- 日志长度大于flush_limit时,会将msg信息推送一次
        flush_limit = 1,
        drop_limit = 99999,
    }
    if not ok then,
...
这里有个新问题，如果 log 里面需要输出一些 content 的临时变量，两阶段之间如何传递参数呢？
